@extends('layouts.app')

@section('content')
<link href="https://fonts.googleapis.com/css2?family=Outfit:wght@400;600;900&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.0/font/bootstrap-icons.css">

<style>
    body { background-color: #f3f7f9; font-family: 'Outfit', sans-serif; }

    .dashboard-wrapper {
        display: flex; min-height: 90vh; margin: 20px;
        background: #ffffff; border-radius: 40px;
        overflow: hidden; box-shadow: 0 20px 60px rgba(0,0,0,0.05);
    }

    .sidebar {
        width: 280px; background-color: #fcfdfe;
        padding: 40px 20px; border-right: 1px solid #f0f0f0;
    }

    .sidebar-item {
        padding: 15px 20px; border-radius: 15px; margin-bottom: 10px;
        color: #8e9297; text-decoration: none; display: flex;
        align-items: center; transition: 0.3s; font-weight: 500;
    }

    .sidebar-item.active { background-color: #fff4e5; color: #ff9f43; font-weight: 600; }

    .main-content { flex: 1; padding: 40px; }

    .task-item {
        background: #ffffff; border-radius: 20px; padding: 18px 25px;
        margin-bottom: 15px; border: 1px solid #f8f9fa;
        display: flex; justify-content: space-between; align-items: center;
        transition: 0.3s; box-shadow: 0 4px 10px rgba(0,0,0,0.02);
    }

    /* Estilo del Checkbox modernizado */
    .task-checkbox {
        width: 22px; height: 22px; cursor: pointer;
        accent-color: #27ae60; /* Color verde al marcar */
    }

    .text-completed { text-decoration: line-through; color: #adb5bd !important; }

    .btn-delete {
        color: #ffbaba; transition: 0.3s; border: none; background: none;
        font-size: 1.2rem;
    }

    .btn-delete:hover { color: #ff4d4d; transform: scale(1.1); }

    .glass-card {
        background: #ffffff; border-radius: 25px; border: 1px solid #f0f0f0;
        padding: 25px; box-shadow: 0 10px 30px rgba(0,0,0,0.02);
    }

    .form-control-custom {
        background-color: #f8fbff; border: 1px solid #eef2f6;
        border-radius: 12px; padding: 12px;
    }

    .btn-schedule {
        background: #c3f2d7; color: #27ae60; border: none;
        border-radius: 15px; padding: 12px; font-weight: 600; width: 100%;
    }
</style>


<div class="dashboard-wrapper">
    <div class="sidebar d-none d-md-block">
        <h4 class="fw-bold mb-5 ps-3">ACTIVIDADES</h4>
        <nav>
            <a href="{{ route('home.index') }}" class="sidebar-item">INICIO</a>
            <a href="{{ route('tasks.index') }}" class="sidebar-item active">TAREAS</a>
            <a href="{{ route('exam.index') }}" class="sidebar-item">EXÁMENES</a>
            <form method="POST" action="{{ route('logout') }}">
    @csrf
    <button type="submit" class="sidebar-item border-0 bg-transparent w-100 text-start" style="cursor: pointer;">
        <i class="bi bi-box-arrow-right me-2"></i> SALIR
    </button>
</form>
                @csrf
            </form>
        </nav>
    </div>

    <div class="main-content">
        <div class="row">
            <div class="col-lg-8">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2 class="fw-bold">TAREAS PROXIMAS</h2>
                    <span class="text-muted">{{ now()->format('d M, Y') }}</span>
                </div>

                @forelse($tasks as $task)
                <div class="task-item" style="{{ $task->status == 'completada' ? 'opacity: 0.6;' : '' }}">
                    <div class="d-flex align-items-center">
                        <input type="checkbox" class="task-checkbox me-3" 
                               {{ $task->status == 'completada' ? 'checked' : '' }}
                               onchange="event.preventDefault(); document.getElementById('update-task-{{ $task->id }}').submit();">
                        
                        <form id="update-task-{{ $task->id }}" action="{{ route('tasks.update', $task) }}" method="POST" style="display: none;">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="status" value="{{ $task->status == 'completada' ? 'pendiente' : 'completada' }}">
                        </form>

                        <div>
                            <p class="mb-0 fw-bold {{ $task->status == 'completada' ? 'text-completed' : 'text-dark' }}">
                                {{ $task->title }}
                            </p>
                            <small class="text-muted">{{ $task->course }}</small>
                        </div>
                    </div>

                    <div class="d-flex align-items-center">
                        <span class="badge me-3" style="background: #f0f7ff; color: #72aee6; border-radius: 10px;">
                            {{ \Carbon\Carbon::parse($task->due_date)->format('d/m') }}
                        </span>
                        
                        <form action="{{ route('tasks.destroy', $task) }}" method="POST" onsubmit="return confirm('¿Eliminar esta tarea?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn-delete">
                                <i class="bi bi-trash3-fill"></i>
                            </button>
                        </form>
                    </div>
                </div>
                @empty
                <p class="text-muted text-center py-5">No hay tareas pendientes.</p>
                @endforelse
            </div>

            <div class="col-lg-4">
                <div class="glass-card">
                    <h5 class="fw-bold mb-4">NUEVA TAREA</h5>
                    <form action="{{ route('tasks.store') }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label class="small fw-bold text-muted">NOMBRE</label>
                            <input type="text" name="title" class="form-control form-control-custom" required>
                        </div>
                        <div class="mb-3">
                            <label class="small fw-bold text-muted">MATERIA</label>
                            <input type="text" name="course" class="form-control form-control-custom" required>
                        </div>
                        <div class="mb-3">
                            <label class="small fw-bold text-muted">FECHA</label>
                            <input type="date" name="due_date" class="form-control form-control-custom" required>
                        </div>
                        <button type="submit" class="btn-schedule mt-2">REGISTRAR</button>
                    </form>
                </div>

                <div class="text-center mt--10">
                    <img src="https://i.pinimg.com/1200x/21/de/8e/21de8e96f3c3ac6cddd707ddfb6368a9.jpg" 
                         alt="Gatito" 
                         style="max-width: 50%; border-radius: 20px; ">
                </div>

            </div>
        </div>
    </div>
</div>
@endsection